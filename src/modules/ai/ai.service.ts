...
export class AiService {
  ...
  async transcribeAudio(audioBuffer: Buffer): Promise<string> {
    ...
    // const tmpFile = tmp.fileSync({ postfix: '.wav' }); // You can adjust the extension
    // const filePath = tmpFile.name;

    // // Write buffer to the temp file
    // fs.writeFileSync(filePath, audioBuffer);

    // // Send file to OpenAI using fs.createReadStream
    // const transcription = await this.openai.audio.transcriptions.create({
    //   file: fs.createReadStream(filePath) as ReadStream,
    //   model: 'whisper-1',
    // });
    ...
    // You need to define filePath here, or use a temp file as in your commented code above
    const tmpFile = tmp.fileSync({ postfix: '.wav' });
    const filePath = tmpFile.name;
    fs.writeFileSync(filePath, audioBuffer);
    form.append('file', fs.createReadStream(filePath), {
      filename: 'audio.wav', // or 'audio.mp3'
      contentType: 'audio/wav', // or your correct MIME type
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to transcribe: ${response.status} - ${error}`);
    }
  ...
  async processOasisData(transcription: string) {
    const oasisPrompt = `
 ...
 M1800: {
  ...
  `;
    ...
    const response = await this.openai.chat.completions.create({
      model: 'gpt-4',
      messages: [{ role: 'user', content: oasisPrompt }],
    });
